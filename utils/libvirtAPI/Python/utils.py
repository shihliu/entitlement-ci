...
class Utils(object):
    ...
    def mac_to_ip(self, mac, timeout=600):
	...
	while timeout > 0:
	    ...
	    timeout -= 10
	    #print str(timeout) + "s left"
			
        return timeout and out or None

    def do_ping(self, ip, timeout=300):
        """Ping some host
    ...
    def scp_file_fromremote(self, hostname, username, password, source_path, target_path, timeout=60):
        """ Scp file from remote host """
        user_hostname = "%s@%s:%s" % (username, hostname, source_path)
        child = pexpect.spawn("/usr/bin/scp", ['-r', user_hostname, target_path], timeout)
        while True:
            index = child.expect(['yes\/no', 'password: ',
                                   pexpect.EOF,
                                   pexpect.TIMEOUT])
