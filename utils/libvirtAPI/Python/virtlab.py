...
def result_log(case_ref_name, case_params, ret, case_start_time, case_end_time):
    # get test run parameters
    global test_run_params
    libvirt_ver = util.get_libvirt_version()
    hypervisor_ver = util.get_hypervisor_version()
    kernel_ver = util.get_host_kernel_version()

    testcase = case_ref_name
    if ret == 0:
        status = 'GOOD'
    else:
        status = 'FAIL'
