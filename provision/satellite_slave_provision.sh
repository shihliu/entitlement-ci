...
#SATELLITE_IP=docker exec -i $CONTAINER_NAME /sbin/ifconfig eth1 | grep "inet addr:" | awk '{print $2}' | cut -c 6-
echo SATELLIE_SLAVE_IP=$SATELLITE_IP>>RESOURCE.txt
echo SATELLIE_SLAVE_NAME=$CONTAINER_NAME>>RESOURCE.txt
# fi
#
echo "Provisioning with the following environment"
echo "-------------------------------------------"
echo "SITE:                     $SITE"
echo "SATELLIE_SLAVE_NAME:      $CONTAINER_NAME"
echo "SATELLIE_SLAVE_IP:             $SATELLITE_IP"
