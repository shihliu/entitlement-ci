...
class tc_ID178195_GUI_reregister_when_registered(RHSMGuiBase):
    ...
    def test_run(self):
        ...
        try:
            try:
                targetmachine_ip = get_exported_param("REMOTE_IP")
                username = RHSMConstants().get_constant("username", targetmachine_ip)
                password = RHSMConstants().get_constant("password", targetmachine_ip)
                self.open_subscription_manager()
                self.check_consumer_cert_files(targetmachine_ip, exist=False)
                ...
                self.check_consumer_cert_files(targetmachine_ip, exist=True)
...
#             self.capture_image(case_name)
            self.restore_gui_environment(targetmachine_ip)
