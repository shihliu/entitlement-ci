...
class rhsm_gui_setup(unittest.TestCase):
    ...
    def rhsm_gui_sys_setup(self):
        ...
        if RHSMConstants().get_os_serials() == "7":
            ...
            ret, output = RHSMConstants().runcmd(cmd, "install gui related packages", showlogger=False)
            if ret == 0:
            ...
            ret, output = RHSMConstants().runcmd(cmd, "install desktop", showlogger=False)
            if ret == 0:
            ...
            ret, output = RHSMConstants().runcmd(cmd, "setup desktop for gui testing", showlogger=False)
            if ret == 0:
        ...
        ret, output = RHSMConstants().runcmd(install_ldtp_cmd, "install ldtp", showlogger=False)
        if ret == 0:
...
EOF
'''
        ret, output = RHSMConstants().runcmd(cmd, "set gnome-terminal desktop", showlogger=False)
        if ret == 0:
        ...
        ret, output = RHSMConstants().runcmd(cmd, "stop iptales")
        if ret == 0:
        ...
        ret, output = RHSMConstants().runcmd(cmd, "check whether vncserver started")
        if ret == 0:
            logger.info("vncserver already started ...")
        else:
            cmd = "vncserver -SecurityTypes None"
            ret, output = RHSMConstants().runcmd(cmd, "start vncserver")
            if ret == 0:
