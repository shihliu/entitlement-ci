...
env=%s''' % (VIRTWHO_ESX_SERVER, VIRTWHO_ESX_USERNAME, VIRTWHO_ESX_PASSWORD, VIRTWHO_ESX_OWNER, VIRTWHO_ESX_ENV)
            ...
            if virtwho_status == "running" or virtwho_status == "active":
                logger.info("Succeeded to check, virt-who is running whit hypervisor_id=hostname.")
            else:
            ...
            if ret == 0 and output is not None and  "ERROR" not in output:
                ...
                if destination_ip in mapping_info and guestuuid in mapping_info:
                    logger.info("Succeeded to check, can find host %s and guest_uuid %s" %(destination_ip, guestuuid))
                else:
                    raise FailException("Failed to check, can not find host %s and guest_uuid %s" %(destination_ip, guestuuid))
            else:
