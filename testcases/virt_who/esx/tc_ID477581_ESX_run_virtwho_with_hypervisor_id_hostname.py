...
class tc_ID477581_ESX_run_virtwho_with_hypervisor_id_hostname(ESXBase):
    def test_run(self):
        ...
        try:
            ...
            self.unset_esx_conf()
            # check esxi hostname
            esxi_hostname = self.esx_get_hostname(destination_ip)
            # creat /etc/virt-who.d/virt.esx file for esxi with hypervisor_id=hostname 
            conf_file = "/etc/virt-who.d/virt.esx"
            self.esx_set_hypervisor_id("hostname", conf_file, esx_owner, esx_env, esx_server, esx_username, esx_password)
            ...
            self.esx_check_host_guest_uuid_exist_in_file(esxi_hostname, guest_uuid, tmp_file)
