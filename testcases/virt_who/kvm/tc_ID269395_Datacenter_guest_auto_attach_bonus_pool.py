        ...
        try:
            ...
            if ret == 0:
                if "Successfully " in output:
                    ...
                    raise FailException("Failed to auto subscribe bonus pool on  %s." % self.get_hg_info(guestip))
            else:
                raise FailException("Failed to auto subscribe to a pool on %s." % self.get_hg_info(guestip))
            # list consumed subscriptions on guest
            self.check_consumed_status(self, guest_bonus_sku, "SubscriptionName", sku_name, guestip)
