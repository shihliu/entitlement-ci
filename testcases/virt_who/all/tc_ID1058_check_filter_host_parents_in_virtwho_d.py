...
class tc_ID1058_check_filter_host_parents_in_virtwho_d(VIRTWHOBase):
    def run_kvm(self):
        try:
            ...
            self.set_filter_host_parents("libvirt", "host_parents", remote_ip_2)
            self.vw_check_message_in_rhsm_log("filter_host_parents is not supported in libvirt mode, ignoring it", message_exists=True, targetmachine_ip=remote_ip_2)
    ...
    def run_remote_libvirt(self):
        try:
            ...
            self.vw_check_message_in_rhsm_log("filter_host_parents is not supported in libvirt mode, ignoring it", message_exists=True)
    ...
    def run_rhevm(self):
        try:
            ...
            # (1) Set Filter_hosts_parents, it will show error info,it will filter host/guest mapping info
            self.set_filter_host_parents("rhevm", "host_parents")
            self.vw_check_message_in_rhsm_log("filter_host_parents is not supported in rhevm mode, ignoring it", message_exists=True)
            self.vw_check_mapping_info_in_rhsm_log(host_uuid, guest_uuid)
        finally:
            self.unset_all_virtwho_d_conf()
            self.set_rhevm_conf()
