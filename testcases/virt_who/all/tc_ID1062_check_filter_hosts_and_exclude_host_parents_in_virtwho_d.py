...
class tc_ID1062_check_filter_hosts_and_exclude_host_parents_in_virtwho_d(VIRTWHOBase):
    def run_kvm(self):
        try:
            ...
            self.set_filter_host_uuids_exclude_parents("libvirt", host_uuid, "host_parents_exclude", remote_ip_2)
            chkmsg = "exclude_host_parents is not supported in libvirt mode, ignoring it"
    ...
    def run_remote_libvirt(self):
        try:
            ...
            # (1) Set Filter_host_uuid and exclude_host_parents, it will show error info,it will filter host/guest mapping info
            self.set_filter_host_uuids_exclude_parents("libvirt", host_uuid, "host_parents_exclude")
            chkmsg = "exclude_host_parents is not supported in libvirt mode, ignoring it"
    ...
    def run_rhevm(self):
        try:
            ...
            # (1) Set Filter_host_uuid and exclude_host_parents, it will show error info,it will filter host/guest mapping info
            self.set_filter_host_uuids_exclude_parents("rhevm", host_uuid, "host_parents_exclude")
            chkmsg = "exclude_host_parents is not supported in rhevm mode, ignoring it"
            self.vw_check_message_in_rhsm_log(chkmsg, message_exists=True)
            self.vw_check_mapping_info_in_rhsm_log(host_uuid, guest_uuid)
        finally:
            self.unset_all_virtwho_d_conf()
    ...
    def run_xen(self):
        try:
            ...
            # (1) Set Filter_host_uuid and exclude_host_parents, it will show error info,it will filter host/guest mapping info
            self.set_filter_host_uuids_exclude_parents("xen", host_uuid, "host_parents_exclude")
            chkmsg = "exclude_host_parents is not supported in xen mode, ignoring it"
            self.vw_check_message_in_rhsm_log(chkmsg, message_exists=True)
            self.vw_check_mapping_info_in_rhsm_log(host_uuid, guest_uuid)
        finally:
            self.unset_all_virtwho_d_conf()
