...
class tc_ID17238_ESX_check_exclude_host_uuids_in_virtwho_d(ESXBase):
    def test_run(self):
        ...
        try:
            ...
            self.esx_set_exclude_host_uuids(host_uuid, esx_owner, esx_env, esx_server, esx_username, esx_password)
            self.vw_check_mapping_info_in_rhsm_log(host_uuid, guest_uuid, uuid_exist=False)
            self.esx_set_exclude_host_uuids("\"\"", esx_owner, esx_env, esx_server, esx_username, esx_password)
            self.vw_check_mapping_info_in_rhsm_log(host_uuid, guest_uuid)
            self.esx_set_exclude_host_uuids("\'\'", esx_owner, esx_env, esx_server, esx_username, esx_password)
            self.vw_check_mapping_info_in_rhsm_log(host_uuid, guest_uuid)
            self.esx_set_exclude_host_uuids("", esx_owner, esx_env, esx_server, esx_username, esx_password)
            self.vw_check_mapping_info_in_rhsm_log(host_uuid, guest_uuid)
            self.esx_set_exclude_host_uuids("\"%s\",\"%s\"" % (host_uuid, host_uuid_second), esx_owner, esx_env, esx_server, esx_username, esx_password)
            self.vw_check_mapping_info_in_rhsm_log(host_uuid, guest_uuid, uuid_exist=False)
