...
class tc_ID17256_RHEVM_check_mapping_when_add_host(VDSMBase):
    def test_run(self):
        ...
        try:
            ...
            # (2) Add host2, check host1 and host2/guest mapping info is exist
            self.rhevm_add_host(host_name_sec, get_exported_param("REMOTE_IP_2"), rhevm_ip)
            host_uuid_sec_after = self.get_host_uuid_on_rhevm(get_exported_param("REMOTE_IP_2"), rhevm_ip)
            self.vw_check_mapping_info_in_rhsm_log(host_uuid_sec_after)
